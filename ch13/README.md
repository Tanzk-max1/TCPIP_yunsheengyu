...
- 带外数据是某些协议（如 TCP）支持的一种紧急数据传输方式。带外数据具有较高的优先级，会立即传输和处理。
- 示例用法：`send(sockfd, buf, len, MSG_OOB);`

**`MSG_DONTROUTE`**：数据包不经过路由处理，直接发送到本地网络。

dont route(不 路由)

- 使用这个标志时，数据包将不会通过系统的路由表发送，只能发送到本地网络中的主机。
- 示例用法：`send(sockfd, buf, len, MSG_DONTROUTE);`

**`MSG_PEEK`**：查看数据但不删除数据。

- 使用这个标志时，数据将从套接字的接收缓冲区读取，但不会从缓冲区中删除。
- 通常用于查看即将到来的数据而不影响后续读取操作。
- 示例用法：`send(sockfd, buf, len, MSG_PEEK);`

**`MSG_WAITALL`**：等待所有数据发送完毕。

- 这个标志表示函数调用不会返回，直到所有指定的数据都发送完毕。
- 注意，使用 `MSG_WAITALL` 时，`send` 函数可能会阻塞，直到所有数据都发送成功或发生错误。
- 示例用法：`send(sockfd, buf, len, MSG_WAITALL);`

**`MSG_NOSIGNAL`**：发送时不产生 SIGPIPE 信号。

- 这个标志在 Linux 系统中常用，当发送的数据到达的套接字已经关闭时，避免产生 SIGPIPE 信号。
- 通常用于防止程序因信号而崩溃。
- 示例用法：`send(sockfd, buf, len, MSG_NOSIGNAL);`
...
1. **发送带外数据** 当使用 `send` 函数发送带外数据时，发送的数据不会与正常数据流按顺序传输，而是通过 TCP 的紧急指针机制传输。带外数据会有一个与正常数据流分离的“紧急”指针来指示其位置。

   ```c++
   send(sock, "4", strlen("4"), MSG_OOB); // 发送紧急数据
   ```

2. **接收带外数据** 接收端使用 `recv` 函数并传递 `MSG_OOB` 标志来接收带外数据。带外数据的偏移量由 TCP 协议自动处理，接收端不需要显式地判断带外数据的位置，只需要在接收时正确处理。

   ```
   
   recv(sock, buf, sizeof(buf), MSG_OOB); // 接收紧急数据
   ```

##### 如何知道偏移量

TCP 的紧急数据机制使用一个 16 位的紧急指针（Urgent Pointer）来指示紧急数据的偏移量。紧急指针的值表示从 TCP 报头的序列号开始的偏移量，指示紧急数据在流中的位置。

- **紧急指针的作用**：发送方设置紧急指针，接收方根据紧急指针确定紧急数据的位置。
...
**`readv` 函数**: 读取操作，可以使用任何文件描述符。例如，可以从标准输入（文件描述符 `0`）、文件描述符 `fd`（打开的文件或套接字）等读取数据。

**`writev` 函数**: 写入操作，可以使用任何文件描述符。例如，可以写入标准输出（文件描述符 `1`）、标准错误（文件描述符 `2`）、文件描述符 `fd`（打开的文件或套接字）等。
