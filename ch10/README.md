...
1. **下列关于进程的说法错误的是？**
   ...
   1. **从操作系统的角度上说，进程是程序运行的单位**：
      - **正确**。操作系统确实将进程视为程序执行的基本单位。每个进程都有其独立的地址空间和系统资源。
   2. **进程根据创建方式建立父子关系**：
      - **正确**。在操作系统中，通常通过调用 `fork` 等系统调用来创建进程。新创建的进程（子进程）与调用 `fork` 的进程（父进程）之间建立父子关系。
   3. **进程可以包含其他进程，即一个进程的内存空间可以包含其他进程**：
      - **错误**。一个进程的内存空间是独立的，不能包含其他进程的内存空间。每个进程都有自己独立的地址空间，不同进程之间不能直接访问彼此的内存。
      - **正确答案**：进程是独立的执行单元，每个进程有独立的地址空间，进程之间不能直接访问对方的内存空间。
   4. **子进程可以创建其他子进程，而创建出来的子进程还可以创建其他子进程，但所有这些进程只与一个父进程建立父子关系**：
      - **错误**。虽然子进程可以创建其他子进程，但每个子进程与其直接创建的进程（即父进程）建立父子关系。这意味着创建一个子进程的进程是该子进程的父进程，而子进程创建的子进程将有它自己的父进程。
      - **正确答案**：子进程可以创建其他子进程，每个进程都有自己的父进程，与直接创建它的进程建立父子关系。
   ...
   **错误原因和正确答案**：
   ...
   1. 进程的内存空间是独立的，不能包含其他进程。
      - **正确答案**：进程是独立的执行单元，每个进程有独立的地址空间，进程之间不能直接访问对方的内存空间。
   2. 子进程和其创建的子进程都有独立的父子关系，不是所有进程都只与一个父进程建立关系。
      - **正确答案**：子进程可以创建其他子进程，每个进程都有自己的父进程，与直接创建它的进程建立父子关系。
...
2. **调用 fork 函数将创建子进程，一下关于子进程错误的是？**
   ...
   1. 让我们逐条分析这些语句：
      ...
      1. **父进程销毁时也会同时销毁子进程**：
         - **错误**。父进程的销毁并不会自动销毁子进程。相反，子进程会被重新分配给 `init` 进程（在 Linux 中为 `PID 1`），以确保子进程的继续执行。
         - **正确答案**：父进程销毁时不会自动销毁子进程。子进程会被重新分配给系统的 `init` 进程。

      2. **子进程是复制父进程所有资源创建出的进程**：
         - **正确**。当调用 `fork` 时，子进程是父进程的一个复制品，拥有父进程的几乎所有资源的副本（如文件描述符、环境变量等）。但它们在不同的地址空间中运行。
         - **正确答案**：子进程是父进程的几乎所有资源的副本，但它们在独立的地址空间中运行。

      3. **父子进程共享全局变量**：
         - **错误**。父子进程在 `fork` 后各自有独立的地址空间，尽管全局变量在子进程中复制，但它们不共享内存。父进程和子进程对全局变量的修改不会相互影响。
         - **正确答案**：父子进程有各自独立的地址空间，全局变量在 `fork` 后被复制，但修改不共享。

      4. **通过 `fork` 函数创建的子进程将执行从开始到 `fork` 函数调用为止的代码**：
         - **错误**。通过 `fork` 函数创建的子进程从 `fork` 函数返回时开始执行，而不是从程序开始到 `fork` 调用为止的代码。
         - **正确答案**：通过 `fork` 函数创建的子进程从 `fork` 函数返回时开始执行，从父进程 `fork` 调用处继续执行。
      ...
      **错误原因和正确答案**：
      ...
      1. 父进程销毁时不会自动销毁子进程。子进程会被重新分配给系统的 `init` 进程。
         - **正确答案**：父进程销毁时不会自动销毁子进程。子进程会被重新分配给系统的 `init` 进程。

      3. 父子进程有各自独立的地址空间，全局变量在 `fork` 后被复制，但修改不共享。
         - **正确答案**：父子进程有各自独立的地址空间，全局变量在 `fork` 后被复制，但修改不共享。

      4. 通过 `fork` 函数创建的子进程从 `fork` 函数返回时开始执行，从父进程 `fork` 调用处继续执行。
         - **正确答案**：通过 `fork` 函数创建的子进程从 `fork` 函数返回时开始执行，从父进程 `fork` 调用处继续执行。
